...
const AddVlog = () => {
  ...
  const validate = () => {
    ...
    else if (formData.creatorName.length < 2)
      newErrors.creatorName = "Creator name must be at least 2 characters";

    if (!formData.title.trim()) newErrors.title = "Vlog title is required";
    else if (formData.title.length < 5)
      newErrors.title = "Vlog title must be at least 5 characters";
    ...
    if (!formData.videoUrl.trim()) newErrors.videoUrl = "Video URL is required";
      ...
      newErrors.videoUrl = "Video URL must start with https://";
    else {
      const match = formData.videoUrl.match(
        /(?:youtube\.com\/(?:watch\?v=|embed\/)|youtu\.be\/)([^&?/]+)/,
      );
      if (!match || !match[1])
        newErrors.videoUrl = "Please provide a valid YouTube video link";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setGeneralError(
        err.message || "Failed to submit vlog. Please try again.",
      );
    } finally {
      ...
      <div className="container">
        <div className="form-wrapper">
          ...
          <p className="form-description">
            Share your latest video with the community
          </p>
          ...
          <form className="vlog-form" onSubmit={handleSubmit}>
            <div className="form-group">
              ...
              />
              {errors.creatorName && (
                <span className="error-message">{errors.creatorName}</span>
              )}
            </div>

            <div className="form-group">
              <label htmlFor="vlogTitle" className="form-label">
                Vlog Title
              </label>
              <input
                type="text"
